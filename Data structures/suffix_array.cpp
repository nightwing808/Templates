...
struct SuffixArray{
    // rank[i] is the postion of the subtring(i,n) is suffix array
    // sa[i] is the starting position of ith smallest suffix
    // lcp[i] is the longest common prefix between ith and (i+1)th index
    // evrything is 0-based
    // STRING CANNOT HAVE CHARACTERS WITH ASCII VALUE LOWER THAN ' '
    // ASCII of ' '=32
    // Just add '|' in between if there are multiple strings
    int n;
    string s;
    ...
    void print(){
        for (int i=0;i<n;i++){
            cout<<sa[i]<<"\t"<<lcp[i]<<"\t"<<s.substr(sa[i],n)<<nl;
        }
    }
    void build(){
        int sz=n;
    ...
    int querysa(int l,int r){ // query sa[l], ..., sa[r]
        int sz=r-l+1;
        int msb=31-__builtin_clz(sz);
        int ans=min(t[l][msb],t[r-(1<<msb)+1][msb]);
        return ans;
    }
    int querylcp(int l,int r){ // query lcp[l], ..., lcp[r]
        int sz=r-l+1;
        int msb=31-__builtin_clz(sz);
    ...
    //      returns 1st position of it's occurence in suffix array
    // else,
    //      returns position of suffix array where it should've been
    ...
    // // if (t is a substring of s),
    //      returns (last+1)th position of it's occurence in suffix array
    // else,
    //      returns position of suffix array where it should've been
    ...
    // occurrences of s[p,...,p+len-1] in the suffix array
    // which is equal to the count of occurence of that substring in the main string
